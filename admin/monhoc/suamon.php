...
// Lay data
    ...
    $data['Sotinchi'] = isset($_POST['heso']) ? $_POST['heso'] : '';
    ...
    if (empty($data['Sotinchi'])){
        $errors['Sotinchi'] = 'Nhập số tín chỉ môn học';
    }

    // Neu ko co loi thi insert
    if (!$errors){
        $mon=$con->edit($data['MaMonHoc'], $data['TenMonHoc'], $data['SoTiet'], $data['Sotinchi']);
...
<form method="post" action="suamon.php?id=<?php echo $data['MaMonHoc']; ?>">
    <table width="50%" border="1" cellspacing="0" cellpadding="10">
        ...
        <tr>
            <td>Số Tín Chỉ</td>
            <td>
                <input type="text" name="heso" value="<?php echo $data['Sotinchi']; ?>"/>
